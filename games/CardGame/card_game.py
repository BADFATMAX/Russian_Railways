...
class Application(tk.Frame):
	def __init__(self, master=None):
		...
		self.current = 1
		self.order = [i for i in range(1, 17)]
		...
		for directory in os.listdir(os.path.join(DIRNAME, 'images')):
			if directory.startswith('rail_map_'):
				...
				self.solDict[directory] = PhotoImage(file=os.path.join(DIRNAME, f'images/rail_map_{directory.split("_")[2]}_resized.png'))
	...
	def draw_header(self):
		...
		directories = [directory for directory in os.listdir(os.path.join(DIRNAME, 'images')) if os.path.isdir(os.path.join(os.path.join(DIRNAME, 'images'), directory))]
	...
	def create_board(self, im_type):
		...
		for index, img in enumerate(self.imgMatrix):
			...
			lbl.grid()
			lbl.bind('<Button-1>', lambda event, pos=index: self.click(pos))
	...
	def check_order(self):
		order = [cell['text'] for cell in self.gridCells]
		if order == [str(i) for i in range(1, 17)]:
	...
	def swap_cell(self, p1, p2):
		...
		self.gridCells[p1]['text'] = str(self.array[p1])
		self.gridCells[p2]['text'] = str(self.array[p2])
...
#
# 	app = Application(icons = {"white_bg": white_bg, "refresh_icon": refresh_icon, "hint_icon": hint_icon, "solved_icon": solved_icon, "logo_image": logo_image}, master=root)
# 	app.mainloop()
...
if __name__ == '__main__':
	...
	white_bg = PhotoImage(file=os.path.join(DIRNAME, 'icons/white_bg.png'))
	refresh_icon = PhotoImage(file=os.path.join(DIRNAME, 'icons/refresh.png'))
	hint_icon = PhotoImage(file=os.path.join(DIRNAME, 'icons/hint.png'))
	solved_icon = PhotoImage(file=os.path.join(DIRNAME, 'icons/solved.png'))
	logo_image = PhotoImage(file=os.path.join(DIRNAME, 'icons/white_bg_1.png'))
	#gif = tk.PhotoImage(file='icons/cga_train2.gif')
